...
class RedoQueue : EventQueue() {
    override fun handleEvent(frame: EventFrame, vi: ViewInitializer, list: MapList, manager: Manager) {
        while (!frame.empty()) {
            ...
            if (event is ModularEvent) {
                ...
                q.handleEvent(frame, Sender.REDO_QUEUE)
            } else if (event.eventType == EventTypes.Edit) {
                val editUpdater = TextUpdateHandler(manager, vi, list)
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(RedoQueue::class.java)
    }
}