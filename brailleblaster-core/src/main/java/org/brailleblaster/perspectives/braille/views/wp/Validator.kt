...
class Validator(var manager: Manager, var view: StyledText) {
    ...
    fun validCut(
        ...
        if (currentElement is PageIndicatorTextMapElement || currentElement is BoxLineTextMapElement) {
            return !(selectionStart == currentStart && selectionLength == currentEnd - currentStart) && if (selectionStart in currentStart until currentEnd && selectionLength <= currentEnd - selectionStart) false else selectionStart != currentEnd || selectionLength != 1 || selectionStart + selectionLength != nextStart
    ...
    fun validPaste(
        ...
        val currentEnd = stateObj.currentEnd
        if (currentElement is PageIndicatorTextMapElement || currentElement is BoxLineTextMapElement) {
            return !(selectionStart == currentStart && selectionLength == currentEnd - currentStart) && if (selectionStart in currentStart until currentEnd && selectionLength <= currentEnd - selectionStart) false else view.selectionRanges[1] != 0 && (selectionStart !in currentStart..currentEnd)
        } else if (selectionLength > 0) {
            val t = manager.mapList.getElementInRange(selectionStart)
            return t !is BoxLineTextMapElement && t !is PageIndicatorTextMapElement || selectionStart != t.getEnd(
                manager.mapList
    ...
    fun validDelete(
        ...
        if (currentElement is Uneditable) {
            return !(selectionStart >= currentStart && selectionStart + selectionLength <= currentEnd || selectionLength == 0) && if (selectionLength <= 0 && view.caretOffset == currentEnd) {
                false
    ...
    fun validBackspace(
        ...
        val nextStart = stateObj.nextStart
        if (currentElement is Uneditable) {
            return !(selectionStart >= currentStart && selectionStart + selectionLength <= currentEnd) && if (selectionLength <= 0) {
                false
