...
import org.brailleblaster.bbx.findBlockOrContainer
...
object MathEditHandler {
    ...
    fun makeMathFromTextViewSelection() {
        ...
        val arrayWithStrings =
            array.map { it to m.textView.text.substring(it.getStart(m.mapList), it.getEnd(m.mapList)) }
        ...
        if (!MathModuleUtils.isMath(mapElement.node)) {
            ...
            if (mapElement.node == null || mapElement is LineBreakElement) {
                ...
                if (m.simpleManager.currentCaret.cursorPosition == CursorPosition.BEFORE) {
                    ...
                    if (previousTme != null) {
                        val previousBlock: Node = previousTme.node.findBlockOrContainer()
                        ...
                        val blockParent = m.simpleManager.currentCaret.node.findBlockOrContainer().parent
                        block = BBX.BLOCK.DEFAULT.create()
                    ...
                    if (nextTme != null) {
                        val nextBlock: Node = nextTme.node.findBlockOrContainer()
                        val blockParent = nextBlock.parent
                        newBlockIndex = blockParent.indexOf(nextBlock)
                        block = BBX.BLOCK.DEFAULT.create()
                        Utils.insertChildCountSafe(blockParent, block, newBlockIndex)
                    } else {
                        val blockOrContainer = m.simpleManager.currentCaret.node.findBlockOrContainer()
