...
class UTDManager @JvmOverloads constructor(styleDefs: StyleDefinitions = loadStyleDefinitions(preferredFormatStandard)) {
    ...
    init {
        ...
        if (translator.dataPath == null) {
            ...
            val programTablesPath =
                BBIni.programDataPath.resolve(Paths.get("org", "mwhapples", "jlouis", "tables")).toString()
...
//			case NEW_PAGES_BEFORE:
    ...
    fun applyBrailleStandard(std: String) {
        engine.brailleSettings = UTDConfig.loadBrailleSettings(
            ...
            ) ?: BBIni.loadAutoProgramDataFileOrNull(UTD_FOLDER, "legacy", std + BRAILLE_SETTINGS_NAME)
            ?: throw RuntimeException("Braille standard file not found.")
        )
        val quotesTable = BBIni.programDataPath.resolve(
            Paths.get(
                "quotesTables",
    ...
    companion object {
        ...
        fun getBaseStyle(style: Style): Style {
            return generateSequence(style) { it.baseStyle }.firstOrNull { !it.name.startsWith(DOCUMENT_STYLE_NAME_PREFIX) }
                ?: style
        }
